...
export async function GET(req: NextRequest) {
  try {
    ...
    // Now fetch the failed assessment applications (directly from the grid results endpoint)
    const legacyResponse = await fetch(`${LEGACY_BASE_URL}/index.jsp?seq=applicantGridResults&applicationName=ConsolidatedJAFTestClientdsiRAT&locale=en_US&reportType=PreScreenedFailed&exclusive=Exclusive.exclusive&hmcBidStatus=JobBidStatusesForHMC.Prescreened&event=com.deploy.application.hmc.plugin.ApplicantGrid.search`, {
      method: 'GET',
      headers: {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8',
        'Accept-Language': 'en-US,en;q=0.5',
