...
export default function TextApplyPage({ fallbackMode = false }: TextApplyPageProps) {
  ...
  useEffect(() => {
    const fetchProspects = async () => {
      try {
        ...
        const response = await fetch('/api/applications/prospects/text-apply')
        
        if (!response.ok) {
          throw new Error(`Failed to fetch text-apply prospects: ${response.status}`)
        }
        
        const data = await response.json()
        setProspects(data)
      } catch (err) {
        console.error('Error fetching text-apply prospects:', err)
      ...
      <div className="flex flex-col h-full">
        <div className="bg-white border-b border-orange-500/20 py-4 px-6 flex items-center justify-between">
          <div className="flex items-center">
            <h1 className="text-2xl font-semibold text-orange-600 mr-8">Text Apply Prospects</h1>
            <p className="text-white/80 text-sm hidden md:block">View and manage text-apply prospects</p>
          </div>
        </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-6 pt-0">
        ...
        <CommonProspectsTable 
          prospects={prospects} 
          title="Text Apply Prospects"
          emptyMessage="No text-apply prospects found"
        />
      </div>
    </div>
  )
